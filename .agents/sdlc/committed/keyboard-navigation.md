...
## Acceptance Criteria
- [ ] Cmd+K / Cmd+P opens the command bar only when global shortcuts are allowed (FocusManager area is graph/none).
- [ ] Command bar supports type-to-filter, arrow key navigation, and Enter to execute commands.
- [ ] Commands include: New session, Jump to latest, Switch project, Set time filter, Close most recent pane, Clear selection.
- [ ] ESC closes the palette and restores focus to the previous element.
- [ ] No conflicts with existing global key handlers (graph navigation remains intact).
